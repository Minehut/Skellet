...
public class RegeneratorManager {
    ...
    public static void regenerate(String ID) {
        if (regenerators.containsKey(ID)) {
            ...
            if (false) { //Skellett.instance.getConfig().getBoolean("Async", false)) {
                throw new UnsupportedOperationException("ASYNC regenerate not supported");
				/*
				List<Location> locs = new ArrayList<>(hashy.keySet());
